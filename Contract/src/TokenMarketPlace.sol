...
contract YouTubeMarketplace is Ownable {
    ...
    event TokenListed(address indexed token, address indexed seller, uint256 price, uint256 amount);
    event TokenSold(address indexed token, address indexed buyer, address indexed seller, uint256 amount, uint256 price);
    ...
    /**
     * @dev Lists a YouTube token for sale.
     * @param token Address of the token to list.
     * @param price Price per token (in WEI).
     * @param amount Number of tokens to sell.
     */
    function listTokenForSale(address token, uint256 price, uint256 amount) external {
    ...
    listings[token][msg.sender] = Listing(msg.sender, price, amount);

    emit TokenListed(token, msg.sender, price, amount);
}

    /**
     * @dev Allows a buyer to purchase a token listing.
    ...
    function buyToken(address token, address seller, uint256 amount) external payable {
        ...
        uint256 royaltyAmount = (msg.value * ytToken.royaltyFee()) / 100;
        ...
        emit TokenSold(token, msg.sender, seller, amount, listing.price);
    }

    /**
     * @dev Returns the total price for a given token listing.
     * @param seller Address of the seller.
     * @param token Address of the token.
    ...
    /**
     * @dev Returns the price per token for a specific listing.
     * @param seller Address of the seller.
     * @param token Address of the token.
